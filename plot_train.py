...
def apply_heatmap(image,data):
    ...
    # 开始绘制热度图
    hm = HeatMap(data)
    hit_img = hm.heatmap(base=background, r = 10) # background为背景图片，r是半径，默认为10
    # plt.figure()
    # plt.imshow(hit_img)
    # plt.show()
...
def plot_att(img_tmp,output):
    ...
    for i in list_1:
        ...
        list_2 += list(att_l)
        for l_xy in list_2:
            recover_w = int(l_xy[0] * b)
            recover_h = int(l_xy[1] * a)
            resize_list.append((recover_w, recover_h))
...
class Trainer(object):
    def __init__(self, args):
        ...
        train_params = [{'params': model.get_1x_lr_params(), 'lr': 0},
                        {'params': model.get_10x_lr_params(), 'lr': 0},
                        {'params': model.get_100x_lr_params(), 'lr': args.lr * 10}]
        ...
        if args.resume is not None:
            ...
            if args.cuda:
                ...
                checkpoint['state_dict'] = {k: v for k, v in checkpoint['state_dict'].items() if k in model_dic}
                model_dic.update(checkpoint['state_dict'])
                self.model.module.load_state_dict(model_dic)
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            tbar.set_description('Train loss: {},Train loss_1: {}'.format(str(train_loss), str(train_loss_1)))
    ...
    def test(self, epoch,viz_label =False):
        self.model.eval()
        self.evaluator.reset()
        tbar = tqdm(self.val_loader, desc='\r')
        test_loss = 0.0
        ...
        for i, sample in enumerate(tbar):
            ...
            if viz_label == True:
                for img, lt, lp in zip(image, target, pred):
                    ...
                    resize_pred = cv2.resize(resize_pred_1, output[3])[:,:,np.newaxis]
                    list_1 = {}
                    for class_index in range(1,20):
                        index_class = np.where(resize_pred==i)
                        class_list = np.column_stack((index_class[1],index_class[0]))
                        if len(class_list)>20:
                            list_1[str(class_index)] = []
                            for i_class in class_list:
                                list_1[str(class_index)].append(i_class)
                    # list_1 = [(0, 0)]
                    # for i in range(w):
                    #     for j in range(h):
                    #         heatmap[(i, j)] = []
                    ...
                    att_l = []
                    list_2 = []
                    for i_index in list_1:
                        resize_list = []
                        att_l = []
                        list_2 = []
                        for i_class in list_1[i_index]:
                            # list_2.append(i_class)
                            i_class_att = img_att[tuple(i_class)]
                            att_l += i_class_att
                        att_l = set(att_l)
...
def main():
    ...
    trainer.test(trainer.args.start_epoch, True)
    for epoch in range(trainer.args.start_epoch, trainer.args.epochs):
        trainer.training(epoch)
        if not trainer.args.no_val and epoch % args.eval_interval == (args.eval_interval - 1):
            trainer.test(epoch,True)
