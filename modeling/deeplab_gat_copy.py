    ...
    def forward(self, input):
        ...
        if x.size()[0]>1:
            for i in range(x.size()[0]-1):
                ...
                x_i = x_i.permute(1, 0)
                # print(x_i.size())
                x_i,img_att_2 = self.gatt(w,h,x_i)
                x_i = x_i.view(1, self.num_class, h, w)
                x_0 = torch.cat([x_0,F.interpolate(x_i, size=input.size()[2:], mode='bilinear', align_corners=True)],dim=0)
        # x_0=x_0+x_1
